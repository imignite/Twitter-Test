...
#step 1
	fiat.tweets = searchTwitter('@fiatamerica', n=5000)
#step 2
	length(fiat.tweets)
	class(fiat.tweets)
	tweet = fiat.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	fiat.text = lapply(fiat.tweets, function(t) t$getText() )
	length(fiat.text)
	head(fiat.text, 5)
	class(fiat.text)
...
fiat.scores = score.sentiment(fiat.text, pos.words,
								neg.words, .progress='text')
fiat.scores$airline = 'fiat'
hist(fiat.scores$score)
