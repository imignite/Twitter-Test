...
#step 1
	infiniti.tweets = searchTwitter('@infinitinews', n=5000)
#step 2
	length(infiniti.tweets)
	class(infiniti.tweets)
	tweet = infiniti.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	infiniti.text = lapply(infiniti.tweets, function(t) t$getText() )
	length(infiniti.text)
	head(infiniti.text, 5)
	class(infiniti.text)
...
infiniti.scores = score.sentiment(infiniti.text, pos.words,
								neg.words, .progress='text')
infiniti.scores$airline = 'infiniti'
hist(infiniti.scores$score)
