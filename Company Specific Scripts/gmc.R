...
#step 1
	gmc.tweets = searchTwitter('@ThisIsGMC', n=5000)
#step 2
	length(gmc.tweets)
	class(gmc.tweets)
	tweet = gmc.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	gmc.text = lapply(gmc.tweets, function(t) t$getText() )
	length(gmc.text)
	head(gmc.text, 5)
	class(gmc.text)
...
gmc.scores = score.sentiment(gmc.text, pos.words,
								neg.words, .progress='text')
gmc.scores$airline = 'gmc'
hist(gmc.scores$score)
