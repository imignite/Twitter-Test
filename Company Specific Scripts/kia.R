...
#step 1
	kia.tweets = searchTwitter('@Kia', n=5000)
#step 2
	length(kia.tweets)
	class(kia.tweets)
	tweet = kia.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	kia.text = lapply(kia.tweets, function(t) t$getText() )
	length(kia.text)
	head(kia.text, 5)
	class(kia.text)
...
kia.scores = score.sentiment(kia.text, pos.words,
								neg.words, .progress='text')
kia.scores$airline = 'kia'
hist(kia.scores$score)
