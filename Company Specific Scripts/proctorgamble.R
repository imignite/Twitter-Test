...
#brand bank
	brand<-c('@ProcterGamble','Procter & Gamble', Procter and Gamble',
			'procter & gamble','proctor and gamble')
#step 1
	pgamb.tweets = searchTwitter(brand, since='2012-08-17', until='2012-08-22')
#step 2
	length(pgamb.tweets)
	class(pgamb.tweets)
	tweet = pgamb.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	pgamb.text = lapply(pgamb.tweets, function(t) t$getText() )
	length(pgamb.text)
	head(pgamb.text, 5)
	class(pgamb.text)
...
pgamb.scores = score.sentiment(pgamb.text, pos.words,
								neg.words, .progress='text')
pgamb.scores$airline = 'pgamb'
hist(pgamb.scores$score)
