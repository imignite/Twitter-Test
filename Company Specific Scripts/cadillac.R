...
#step 1
	cadillac.tweets = searchTwitter('@Cadillac', n=5000)
#step 2
	length(cadillac.tweets)
	class(cadillac.tweets)
	tweet = cadillac.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	cadillac.text = lapply(cadillac.tweets, function(t) t$getText() )
	length(cadillac.text)
	head(cadillac.text, 5)
	class(cadillac.text)
...
cadillac.scores = score.sentiment(cadillac.text, pos.words,
								neg.words, .progress='text')
cadillac.scores$airline = 'cadillac'
hist(cadillac.scores$score)
