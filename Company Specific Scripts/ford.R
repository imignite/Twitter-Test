...
#step 1
	ford.tweets = searchTwitter('@Ford', n=5000)
#step 2
	length(ford.tweets)
	class(ford.tweets)
	tweet = ford.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	ford.text = lapply(ford.tweets, function(t) t$getText() )
	length(ford.text)
	head(ford.text, 5)
	class(ford.text)
...
ford.scores = score.sentiment(ford.text, pos.words,
								neg.words, .progress='text')
ford.scores$airline = 'ford'
hist(ford.scores$score)
