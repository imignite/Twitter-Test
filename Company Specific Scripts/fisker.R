...
#step 1
	fisker.tweets = searchTwitter('@fiskerauto', n=5000)
#step 2
	length(fisker.tweets)
	class(fisker.tweets)
	tweet = fisker.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	fisker.text = lapply(fisker.tweets, function(t) t$getText() )
	length(fisker.text)
	head(fisker.text, 5)
	class(fisker.text)
...
fisker.scores = score.sentiment(fisker.text, pos.words,
								neg.words, .progress='text')
fisker.scores$airline = 'fisker'
hist(fisker.scores$score)
