...
#step 1
	jaguar.tweets = searchTwitter('@JaguarUSA', n=5000)
#step 2
	length(jaguar.tweets)
	class(jaguar.tweets)
	tweet = jaguar.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	jaguar.text = lapply(jaguar.tweets, function(t) t$getText() )
	length(jaguar.text)
	head(jaguar.text, 5)
	class(jaguar.text)
...
jaguar.scores = score.sentiment(jaguar.text, pos.words,
								neg.words, .progress='text')
jaguar.scores$airline = 'jaguar'
hist(jaguar.scores$score)
