...
#step 1
	scion.tweets = searchTwitter('@scion', n=5000)
#step 2
	length(scion.tweets)
	class(scion.tweets)
	tweet = scion.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	scion.text = lapply(scion.tweets, function(t) t$getText() )
	length(scion.text)
	head(scion.text, 5)
	class(scion.text)
...
scion.scores = score.sentiment(scion.text, pos.words,
								neg.words, .progress='text')
scion.scores$airline = 'scion'
hist(scion.scores$score)
