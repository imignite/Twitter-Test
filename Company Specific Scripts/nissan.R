...
#step 1
	nissan.tweets = searchTwitter('@NissanNews', n=5000)
#step 2
	length(nissan.tweets)
	class(nissan.tweets)
	tweet = nissan.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	nissan.text = lapply(nissan.tweets, function(t) t$getText() )
	length(nissan.text)
	head(nissan.text, 5)
	class(nissan.text)
...
nissan.scores = score.sentiment(nissan.text, pos.words,
								neg.words, .progress='text')
nissan.scores$airline = 'nissan'
hist(nissan.scores$score)
