...
#step 1
	honda.tweets = searchTwitter('@Honda', n=5000)
#step 2
	length(honda.tweets)
	class(honda.tweets)
	tweet = honda.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	honda.text = lapply(honda.tweets, function(t) t$getText() )
	length(honda.text)
	head(honda.text, 5)
	class(honda.text)
...
honda.scores = score.sentiment(honda.text, pos.words,
								neg.words, .progress='text')
honda.scores$airline = 'honda'
hist(honda.scores$score)
