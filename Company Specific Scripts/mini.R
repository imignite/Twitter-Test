...
#step 1
	mini.tweets = searchTwitter('@MINI', n=5000)
#step 2
	length(mini.tweets)
	class(mini.tweets)
	tweet = mini.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	mini.text = lapply(mini.tweets, function(t) t$getText() )
	length(mini.text)
	head(mini.text, 5)
	class(mini.text)
...
mini.scores = score.sentiment(mini.text, pos.words,
								neg.words, .progress='text')
mini.scores$airline = 'mini'
hist(mini.scores$score)
