...
#step 1
	porsche.tweets = searchTwitter('@Porsche', n=5000)
#step 2
	length(porsche.tweets)
	class(porsche.tweets)
	tweet = porsche.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	porsche.text = lapply(porsche.tweets, function(t) t$getText() )
	length(porsche.text)
	head(porsche.text, 5)
	class(porsche.text)
...
porsche.scores = score.sentiment(porsche.text, pos.words,
								neg.words, .progress='text')
porsche.scores$airline = 'porsche'
hist(porsche.scores$score)
