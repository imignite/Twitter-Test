...
#step 1
	bentley.tweets = searchTwitter('@BentleyMotors', n=5000)
#step 2
	length(bentley.tweets)
	class(bentley.tweets)
	tweet = bentley.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	bentley.text = lapply(bentley.tweets, function(t) t$getText() )
	length(bentley.text)
	head(bentley.text, 5)
	class(bentley.text)
...
bentley.scores = score.sentiment(bentley.text, pos.words,
								neg.words, .progress='text')
bentley.scores$airline = 'bentley'
hist(bentley.scores$score)
