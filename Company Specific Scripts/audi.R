...
#step 1
	audi.tweets = searchTwitter('@Audi', n=5000)
#step 2
	length(audi.tweets)
	class(audi.tweets)
	tweet = audi.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	audi.text = lapply(audi.tweets, function(t) t$getText() )
	length(audi.text)
	head(audi.text, 5)
	class(audi.text)
...
audi.scores = score.sentiment(audi.text, pos.words,
								neg.words, .progress='text')
audi.scores$airline = 'audi'
hist(audi.scores$score)
