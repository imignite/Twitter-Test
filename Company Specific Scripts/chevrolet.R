...
#step 1
	chevrolet.tweets = searchTwitter('@chevrolet', n=5000)
#step 2
	length(chevrolet.tweets)
	class(chevrolet.tweets)
	tweet = chevrolet.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	chevrolet.text = lapply(chevrolet.tweets, function(t) t$getText() )
	length(chevrolet.text)
	head(chevrolet.text, 5)
	class(chevrolet.text)
...
chevrolet.scores = score.sentiment(chevrolet.text, pos.words,
								neg.words, .progress='text')
chevrolet.scores$airline = 'chevrolet'
hist(chevrolet.scores$score)
