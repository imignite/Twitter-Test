...
#step 1
	bmw.tweets = searchTwitter('@BMWUSA_news', n=5000)
#step 2
	length(bmw.tweets)
	class(bmw.tweets)
	tweet = bmw.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	bmw.text = lapply(bmw.tweets, function(t) t$getText() )
	length(bmw.text)
	head(bmw.text, 5)
	class(bmw.text)
...
bmw.scores = score.sentiment(bmw.text, pos.words,
								neg.words, .progress='text')
bmw.scores$airline = 'bmw'
hist(bmw.scores$score)
