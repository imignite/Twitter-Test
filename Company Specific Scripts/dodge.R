...
#step 1
	dodge.tweets = searchTwitter('@Dodge', n=5000)
#step 2
	length(dodge.tweets)
	class(dodge.tweets)
	tweet = dodge.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	dodge.text = lapply(dodge.tweets, function(t) t$getText() )
	length(dodge.text)
	head(dodge.text, 5)
	class(dodge.text)
...
dodge.scores = score.sentiment(dodge.text, pos.words,
								neg.words, .progress='text')
dodge.scores$airline = 'dodge'
hist(dodge.scores$score)
