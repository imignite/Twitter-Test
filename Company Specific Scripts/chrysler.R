...
#step 1
	chrysler.tweets = searchTwitter('@Chrysler', n=5000)
#step 2
	length(chrysler.tweets)
	class(chrysler.tweets)
	tweet = chrysler.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	chrysler.text = lapply(chrysler.tweets, function(t) t$getText() )
	length(chrysler.text)
	head(chrysler.text, 5)
	class(chrysler.text)
...
chrysler.scores = score.sentiment(chrysler.text, pos.words,
								neg.words, .progress='text')
chrysler.scores$airline = 'chrysler'
hist(chrysler.scores$score)
