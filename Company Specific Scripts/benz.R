...
#step 1
	benz.tweets = searchTwitter('@MBUSA', n=5000)
#step 2
	length(benz.tweets)
	class(benz.tweets)
	tweet = benz.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	benz.text = lapply(benz.tweets, function(t) t$getText() )
	length(benz.text)
	head(benz.text, 5)
	class(benz.text)
...
benz.scores = score.sentiment(benz.text, pos.words,
								neg.words, .progress='text')
benz.scores$airline = 'benz'
hist(benz.scores$score)
