...
#step 1
	lexus.tweets = searchTwitter('@Lexus', n=5000)
#step 2
	length(lexus.tweets)
	class(lexus.tweets)
	tweet = lexus.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	lexus.text = lapply(lexus.tweets, function(t) t$getText() )
	length(lexus.text)
	head(lexus.text, 5)
	class(lexus.text)
...
lexus.scores = score.sentiment(lexus.text, pos.words,
								neg.words, .progress='text')
lexus.scores$airline = 'lexus'
hist(lexus.scores$score)
