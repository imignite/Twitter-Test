...
#step 1
	mazda.tweets = searchTwitter('@MazdaUSA', n=5000)
#step 2
	length(mazda.tweets)
	class(mazda.tweets)
	tweet = mazda.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	mazda.text = lapply(mazda.tweets, function(t) t$getText() )
	length(mazda.text)
	head(mazda.text, 5)
	class(mazda.text)
...
mazda.scores = score.sentiment(mazda.text, pos.words,
								neg.words, .progress='text')
mazda.scores$airline = 'mazda'
hist(mazda.scores$score)
