...
#step 1
	acura.tweets = searchTwitter('@Acura_Insider', n=5000)
#step 2
	length(acura.tweets)
	class(acura.tweets)
	tweet = acura.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	acura.text = lapply(acura.tweets, function(t) t$getText() )
	length(acura.text)
	head(acura.text, 5)
	class(acura.text)
...
acura.scores = score.sentiment(acura.text, pos.words,
								neg.words, .progress='text')
acura.scores$airline = 'acura'
hist(acura.scores$score)
