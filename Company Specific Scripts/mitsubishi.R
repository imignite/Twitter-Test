...
#step 1
	mitsubishi.tweets = searchTwitter('@mitsucars', n=5000)
#step 2
	length(mitsubishi.tweets)
	class(mitsubishi.tweets)
	tweet = mitsubishi.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	mitsubishi.text = lapply(mitsubishi.tweets, function(t) t$getText() )
	length(mitsubishi.text)
	head(mitsubishi.text, 5)
	class(mitsubishi.text)
...
mitsubishi.scores = score.sentiment(mitsubishi.text, pos.words,
								neg.words, .progress='text')
mitsubishi.scores$airline = 'mitsubishi'
hist(mitsubishi.scores$score)
