...
#step 1
	ramtruck.tweets = searchTwitter('@RamTrucks', n=5000)
#step 2
	length(ramtruck.tweets)
	class(ramtruck.tweets)
	tweet = ramtruck.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	ramtruck.text = lapply(ramtruck.tweets, function(t) t$getText() )
	length(ramtruck.text)
	head(ramtruck.text, 5)
	class(ramtruck.text)
...
ramtruck.scores = score.sentiment(ramtruck.text, pos.words,
								neg.words, .progress='text')
ramtruck.scores$airline = 'ramtruck'
hist(ramtruck.scores$score)
