...
#step 1
	rolls.tweets = searchTwitter('@rollsroycecars', n=5000)
#step 2
	length(rolls.tweets)
	class(rolls.tweets)
	tweet = rolls.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	rolls.text = lapply(rolls.tweets, function(t) t$getText() )
	length(rolls.text)
	head(rolls.text, 5)
	class(rolls.text)
...
rolls.scores = score.sentiment(rolls.text, pos.words,
								neg.words, .progress='text')
rolls.scores$airline = 'rolls'
hist(rolls.scores$score)
