...
#step 1
	lambo.tweets = searchTwitter('@lamborghini', n=5000)
#step 2
	length(lambo.tweets)
	class(lambo.tweets)
	tweet = lambo.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	lambo.text = lapply(lambo.tweets, function(t) t$getText() )
	length(lambo.text)
	head(lambo.text, 5)
	class(lambo.text)
...
lambo.scores = score.sentiment(lambo.text, pos.words,
								neg.words, .progress='text')
lambo.scores$airline = 'lambo'
hist(lambo.scores$score)
