...
#step 1
	hyundai.tweets = searchTwitter('@Hyundai', n=5000)
#step 2
	length(hyundai.tweets)
	class(hyundai.tweets)
	tweet = hyundai.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	hyundai.text = lapply(hyundai.tweets, function(t) t$getText() )
	length(hyundai.text)
	head(hyundai.text, 5)
	class(hyundai.text)
...
hyundai.scores = score.sentiment(hyundai.text, pos.words,
								neg.words, .progress='text')
hyundai.scores$airline = 'hyundai'
hist(hyundai.scores$score)
