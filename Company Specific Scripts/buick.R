...
#step 1
	buick.tweets = searchTwitter('@Buick', n=5000)
#step 2
	length(buick.tweets)
	class(buick.tweets)
	tweet = buick.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	buick.text = lapply(buick.tweets, function(t) t$getText() )
	length(buick.text)
	head(buick.text, 5)
	class(buick.text)
...
buick.scores = score.sentiment(buick.text, pos.words,
								neg.words, .progress='text')
buick.scores$airline = 'buick'
hist(buick.scores$score)
