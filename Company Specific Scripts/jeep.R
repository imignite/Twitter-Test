...
#step 1
	jeep.tweets = searchTwitter('@Jeep', n=5000)
#step 2
	length(jeep.tweets)
	class(jeep.tweets)
	tweet = jeep.tweets[[1]]
	...
	tweet$getText()
#Extracting text
	jeep.text = lapply(jeep.tweets, function(t) t$getText() )
	length(jeep.text)
	head(jeep.text, 5)
	class(jeep.text)
...
jeep.scores = score.sentiment(jeep.text, pos.words,
								neg.words, .progress='text')
jeep.scores$airline = 'jeep'
hist(jeep.scores$score)
