...
#step 1
	landrover.tweets = searchTwitter('@LandRoverUSA', n=5000)
#step 2
	length(landrover.tweets)
	class(landrover.tweets)
	tweet = landrover.tweets[[1]]
	class(tweet)
	tweet$getScreenName()
	tweet$getText()
#Extracting text
	landrover.text = lapply(landrover.tweets, function(t) t$getText() )
	length(landrover.text)
	head(landrover.text, 5)
	class(landrover.text)
...
landrover.scores = score.sentiment(landrover.text, pos.words,
								neg.words, .progress='text')
landrover.scores$airline = 'landrover'
hist(landrover.scores$score)
